...
const StackedBarChart = (props) => {
  ...
  useEffect(() => {
    ...
    const xAxisTop = axisTop(xScale).tickSize(0).ticks(0).tickValues([]);
    ...
    const tootTipHtml = (event) =>
      `<div><p>Date: ${event.target.__data__.data.key}</p><p>Match 1: ${event.target.__data__.data.matchOne}</p><p>Match 2: ${event.target.__data__.data.matchTwo}</p></div>`;

    var mousemove = function (event, d) {
      // console.log('event', event);

      Tooltip.html(tootTipHtml(event))
